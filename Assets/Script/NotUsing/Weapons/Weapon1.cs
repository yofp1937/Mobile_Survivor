...
public class Weapon1 : MonoBehaviour
{
    ...
    public int prefabId;
    float lastATKtime;
    public WeaponData1 data;
    List<WeaponSetting1> rotateSwords;
    ...
    void Start()
    {
        ...
        playerstat = GameManager1.instance.player.GetComponent<Status1>();
        rotateSwords = new List<WeaponSetting1>();
    ...
    void Update()
    {
        ...
        if(Input.GetButtonDown("Jump")){
            var levelupdata = new WeaponData1{ damage = 1, count = 1, speed = 1 };
            LevelUp(levelupdata);
        }
    }
    ...
    void BatchRotateSword() // RotateSword 1 사이클 실행
    {
        for(int index=0; index < data.count + playerstat.Amount; index++){
            ...
            if(index < transform.childCount){ // index값이 Weapon0의 자식 수보다 적으면 생성돼있는 rotatesword를 그대로 사용
                ...
                weaponT = GameManager1.instance.weapon.Get(prefabId).transform;
                weaponT.parent = transform; // rotatesword 만든후 부모를 weapon0으로 설정
                var rotatesword = weaponT.GetComponent<WeaponSetting1>();
            ...
            weaponT.GetComponent<WeaponSetting1>().Init(data.damage * playerstat.Damage, -1, data.knockback, Vector3.zero); // 무한 관통이라 per는 -1로 설정
        }
        AttackRotateSword();
    }
    ...
    IEnumerator AttackThrowWeaponCoroutine()
    {
        for(int i = 0; i < data.count + playerstat.Amount; i++)
        {
            ...
            Transform weaponT = GameManager1.instance.weapon.Get(prefabId).transform;
            weaponT.parent = transform;
            weaponT.position = transform.position;
            weaponT.rotation = Quaternion.FromToRotation(Vector3.up, dir);
            weaponT.GetComponent<WeaponSetting1>().Init(data.damage * playerstat.Damage, 0, data.knockback, dir);
    ...
    public void LevelUp(WeaponData1 weapon)
    {
